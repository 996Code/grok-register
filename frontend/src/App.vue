      ...
      <n-dialog-provider>
        <n-layout style="height: 100vh">
          ...
          <n-layout has-sider style="height: calc(100vh - 56px)">
            <n-layout-sider bordered :width="180" :collapsed-width="0">
...
const menuOptions = [
  { label: '总览控制台', key: 'dashboard', icon: () => '📊' },
  { label: '账号管理', key: 'accounts', icon: () => '📁' },
  { label: 'Grok2API', key: 'grokapi', icon: () => '🔌' },
  { label: '配置', key: 'config', icon: () => '⚙️' },
]
...
onMounted(() => {
  ...
  onUnmounted(() => { eventSource?.close(); clearInterval(poll) })
})
</script>
